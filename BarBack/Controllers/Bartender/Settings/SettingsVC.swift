...
class SettingsVC: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    // MARK: - IB Outlets
    @IBOutlet weak var tbl_settingList: UITableView!
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row == 0 {
            let vc = storyboard?.instantiateViewController(withIdentifier: "GeneralSettingsView") as? GeneralSettingsView
            vc?.model.append(contentsOf: self.profileModel[0].data?.notificationPreference ?? [])
            self.navigationController?.pushViewController(vc!, animated: true)
        }else if indexPath.row == 1 {
            let vc = storyboard?.instantiateViewController(withIdentifier: "PrivacySettingsView") as? PrivacySettingsView
...
extension SettingsVC: SideMenuDelegates {
