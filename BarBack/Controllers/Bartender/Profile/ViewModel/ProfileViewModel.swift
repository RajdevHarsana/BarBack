...
class ProfileViewModel {
    ...
    var controller : ProfileView?
    var model = [ProfileModel]()
    var model1 = [BarProfileModel]()
    ...
    func getProfileAPI(){
        ...
        APIManager.shared.getProfileApi(baseUrl: Config().API_URL + "/api/profile-detail", parameter: paramDict, token: authToken) { response in
            ...
            if data.success ?? false == true {
                ...
                self.model.append(response as! ProfileModel)
                self.updateUI()
    ...
    func userDetailAPI(userId:Int){
        ...
        APIManager.shared.barShiftDetailApi(baseUrl: Config().API_URL + "/api/user-detail", parameter: paramDict, token: authToken) { response in
            ...
            let message = data.message ?? ""
            if data.success ?? false == true {
                self.model1.removeAll()
                self.model1.append(response as! BarProfileModel)
                self.updateUI()
                self.controller?.workExpTbl.reloadData()
                self.controller?.skillsCollection.reloadData()
                self.controller?.languageCollection.reloadData()
