...
class BarEditProfileViewModel {
    
    var controller: BarEditProfileView?
    var placeLat = Double()
    var placeLong = Double()
    var model = [BarUpdateProfileModel]()
    var name = String()
    var email = String()
    var address = String()
    ...
    func editProfileApi(fullName:String,email:String,about:String,address1:String,address2:String,city:String,state:String,zipCode:String,image: UIImage,coverImage:UIImage){
        Loader.start()
        let param = ["fullname":fullName,"email":email,"about_me":about,"address1":address1,"address2":address2,"city":city,"state":state,"country":self.country,"zip":zipCode,"country_code":self.countryCode,"latitude":placeLat,"longitude":placeLong] as [String : Any]
        
        APIManager.shared.barUploadImageDataWithToken(inputUrl: Config().API_URL + "/api/update-profile", parameters: param, imageName: "image", imageFile: image,coverimageName: "cover_image", coverimageFile: coverImage) { response in
            print(response)
            let data = response as! BarUpdateProfileModel
            let message = data.message ?? ""
            if data.status ?? 0 == 200 {
                self.model.append(response as! BarUpdateProfileModel)
