...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let LoginVal = Config().AppUserDefaults.string(forKey: "userLogin")
//        if Config().AppUserDefaults.value(forKey: "hasSeenIntro") != nil{
            if LoginVal == "Login"{
                let userType = Config().AppUserDefaults.string(forKey: "userType")
                ...
                let nav1 = UINavigationController()
                let storyboard = UIStoryboard(name: "Main", bundle: nil)
                let navigate = storyboard.instantiateViewController(withIdentifier: "IntroVC") as! IntroVC
                nav1.viewControllers = [navigate]
                UIApplication.shared.windows.first?.rootViewController = nav1
                UIApplication.shared.windows.first?.makeKeyAndVisible()
            }
